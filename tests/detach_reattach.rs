...
fn pane_created_without_client_uses_configured_headless_size() {
    ...
    // A lone pane spends its top row on the title strip, so the configured
    // headless size reaches the pane one row shorter.
...
fn pane_created_after_detach_uses_configured_headless_size() {
    ...
    // The lone root pane's title strip takes the top row.
