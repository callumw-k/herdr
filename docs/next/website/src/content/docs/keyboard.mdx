...
A floating pane sits above the tab's normal split layout in a centred box, instead of taking a slot in the grid. Press `prefix+f` to open one. Floats stack per tab, and only the topmost one is drawn, so `prefix+ctrl+f` cycles the stack to bring the next float to the top. `prefix+shift+f` hides or shows the whole floating layer for the tab without closing anything underneath it.
...
`prefix+enter` follows the same rule: with the floating layer up it adds another float rather than a tiled pane. When the floating layer holds focus, `focus_pane_up` and `focus_pane_down` move through the float stack instead of the tiled panes, up towards the preview bars and down back to the front.
