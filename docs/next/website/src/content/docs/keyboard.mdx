...
The goto picker opens with the search box focused. Press `esc` once to leave search, then `p` on any row to edit that workspace's pinned path, or `ctrl+o` to create a workspace with a name and path.
