...
| Cycle pane arrangement | `prefix+a` / `prefix+shift+a` |
...
Each tab lays its panes out under one arrangement: even columns, even rows, a grid, or a single stack. `prefix+a` and `prefix+shift+a` cycle through them, rebuilding the tab from its panes in order, so a full lap brings back the layout you started with. [Concepts](/docs/concepts/) describes what each one looks like.
