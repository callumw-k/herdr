...
| Focus or hide the floating layer | `prefix+ctrl+f` |
...
| Agent overview | `prefix+g` |
| Toggle sidebar | `prefix+b` |

The agent overview opens full screen with the search box focused. On wide terminals a live preview of the selected pane sits on the right, with the pane's colours. Scroll it with the mouse wheel over the preview or with `pgup` and `pgdn`. By default it lists only panes with a detected agent. Press `alt+a` in search mode or `a` in tree mode to show everything, and Herdr remembers that choice per machine (`ui.navigator_agents_only` sets the initial value). `tab` switches between search and tree mode, `esc` clears the search or closes when it is already empty, `enter` opens the selected pane, and pressing `prefix+g` again closes it. A search matches pane names, agent kinds, titles, pane ids, paths, and the workspace, tab and branch a pane lives under. In tree mode `p` edits the selected workspace's pinned path, `ctrl+o` creates a workspace with a name and path, and `left`/`right` or `h`/`l` jump to the previous or next workspace. When the tree overflows, a scrollbar appears on its right edge: use the mouse wheel, click the track, or drag the thumb to scroll. Set `ui.navigator_on_start = true` to open the overview as soon as the client attaches.

The workspace dialog has a name and a path field. `tab` switches between them, and the path pins the workspace (see [Concepts](/docs/concepts/)).
...
The floating layer sits above the tab's normal split layout in a centred region, instead of taking slots in the grid. Press `prefix+f` to open a float. A tab's floats share that one region and are laid out inside it under the floating layer's own arrangement, so more than one can be visible at once. The layer starts stacked, which fills the region with the focused float and collapses the others to single rows, and `prefix+]` and `prefix+[` cycle it through the same four arrangements the tiled panes use while the layer holds focus.

`prefix+ctrl+f` combines opening, focusing and hiding into one key: it opens a float when the tab has none, moves focus into the layer when the layer is not focused, and hides the layer when it already holds focus. Nothing is ever closed by this action. `prefix+shift+f` hides or shows the whole floating layer without closing anything.

Once the layer holds focus, walk it with the same keys as tiled panes: `prefix+h/j/k/l` and `prefix+tab`. Directional movement never crosses between the layers, so use the float keys for that. `prefix+enter` needs the floating layer focused, not merely visible, before it adds another float; with a tiled pane focused it adds a tiled pane instead.
