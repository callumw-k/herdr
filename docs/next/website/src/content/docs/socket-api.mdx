...
| Workspace | `workspace.create`, `workspace.list`, `workspace.get`, `workspace.focus`, `workspace.rename`, `workspace.set_path`, `workspace.move`, `workspace.move_block`, `workspace.report_metadata`, `workspace.path_pin_toggle`, `workspace.declared_repo_toggle`, `workspace.close` |
| Worktree | `worktree.list`, `worktree.create`, `worktree.open`, `worktree.remove` |
| Tab | `tab.create`, `tab.list`, `tab.get`, `tab.focus`, `tab.rename`, `tab.move`, `tab.close`, `tab.pane_add`, `tab.arrangement`, `tab.float_activate`, `tab.floats_toggle` |
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.float`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Popup | `popup.close` |
...
`workspace.set_path` sets the workspace's pinned path, the directory it claims for pane routing. Pass `path: null` to clear the pin. The path expands a leading `~` and resolves relative to the server's working directory, and it does not have to exist yet. Every `WorkspaceInfo` carries the resolved absolute `path`, or omits the field when nothing is pinned. `workspace.path_pin_toggle` pins or unpins the focused pane's directory and `workspace.declared_repo_toggle` adds or removes a `[[repos]]` entry in `config.toml`. See [Concepts](/docs/concepts/) for what Herdr does with a pinned path.
...
The response includes `workspace_id`, `tab_id`, `zoomed`, `focused_pane_id`,
`arrangement`, and `root`. `arrangement` is the tab's pane arrangement:
`vertical`, `horizontal`, `grid`, or `stacked`. `root` is a BSP tree of `pane`,
`split`, and `stack` nodes. Pane nodes can include `pane_id`, `label`, `cwd`,
and argv `command`. Split nodes use `direction` (`right` or `down`), `ratio`,
`first`, and `second`. Stack nodes use `panes`, a flat list of pane nodes, and
`active`, the index of the expanded member. Stacks do not nest and cannot
contain a split. `arrangement` describes how the tab will be reflowed; after a
`layout.apply` call with a `Split` root, the tab retains its existing arrangement value until the next pane is created or closed.

The response also includes `float_arrangement` and `float_root` for the tab's
floating layer, a second independent tree of the same `pane`/`split`/`stack`
shape. `float_arrangement` defaults to `stacked` rather than `grid`, and
`float_root` is omitted when the tab has no floats.
...
not preserve live PTYs, scrollback, or running processes. An optional
`float_root` alongside `root` installs the tab's floating layer the same way;
a `Stack` float root sets `float_arrangement` to `stacked`, matching the rule
for `root`.
...
`pane.split` and `pane.move` accept `ratio` (and `split` for a move) for pane
ordering only. A tab reflows under its arrangement after every pane create,
close or move, so the response's `target_layout` shows the tree before that
reflow. `layout.apply` keeps an exact tree until the next create or close.
