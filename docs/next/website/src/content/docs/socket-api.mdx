...
| Workspace | `workspace.create`, `workspace.list`, `workspace.get`, `workspace.focus`, `workspace.rename`, `workspace.set-path`, `workspace.move`, `workspace.move_block`, `workspace.report_metadata`, `workspace.close` |
...
`workspace.set-path` sets the workspace's pinned path, the directory it claims
for pane routing. Pass `path: null` to clear the pin. `workspace.create` takes
the same optional `path`, and falls back to it for `cwd` when you give a path
and no cwd. Both expand a leading `~` and resolve a relative path against the
server's working directory. The directory does not have to exist, so you can
pin a repo before you clone it. Every `WorkspaceInfo` carries the resolved
absolute `path`, or omits the field when nothing is pinned. See
[Concepts](/docs/concepts/) for what Herdr does with a pinned path.
