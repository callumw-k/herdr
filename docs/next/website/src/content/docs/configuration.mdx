...
Entering a declared directory in the focused pane creates a workspace pinned to it and moves the pane there. Nothing is created until you go there, so a fresh session stays empty until you use it. The pane leaves the workspace it was in, and that workspace closes if the pane was its last one.

A declared repo is skipped when a workspace already pins the directory, when the pane's own workspace is already pinned to a path that covers it, when the pane reporting the change is not the one you are focused on, and when the pane is busy with a foreground process or a detected agent. Once created, the workspace is an ordinary pinned workspace: rename it with `prefix+shift+w`, repin it with `prefix+.`, and close it with `prefix+shift+d`. Entering the directory again after closing it creates it again.

`prefix+>` edits the list for you. It adds the active workspace's directory to `[[repos]]`, or removes every entry declaring that directory when it is already there, and it sits next to `prefix+.`, which pins the workspace to that same directory. The workspace's pinned path is used when it has one, otherwise the focused pane's directory. Herdr writes an absolute path and reloads the config, so the entry takes effect straight away. Rebind it as `toggle_declared_repo` under `[keys]`.

Paths accept `~` and need not exist yet. When one declared path sits inside another, the deepest match wins. Declare the repos themselves rather than a parent directory: declaring `path = "~"`, for example, pins a workspace to your home directory, which then claims every idle pane's `cd` in every workspace.
...
Floating panes (`prefix+f`, see [Keyboard](/docs/keyboard/#floating-panes)) default to half the terminal area. Override the size with `floating_pane_width` and `floating_pane_height`, using a cell count or a percentage string:

```toml
[ui]
floating_pane_width = 100
floating_pane_height = "60%"
```
...
Every space and agent entry also draws a ribbon in its first column. The ribbon's color is that entry's agent state and its weight marks the active entry, so the default layouts leave `state_icon` out. In Navigate mode the cursor's ribbon switches to the accent color, which keeps it apart from the active row behind it. Add the token back to a row to show the inline icon as well.
...
- `activity` — the first value present out of `terminal_title_stripped`, `pane`, and `agent`.
...
When the Agent panel is sorted by space, it prints a space name header above each run of agents from the same space and drops the `workspace` token from those agents' rows. A row left with no values disappears, so the default layout renders one header and one line per agent. Priority ordering and custom view sorts keep the `workspace` token and print no headers.
