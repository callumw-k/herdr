...
Every space and agent entry carries a coloured status ribbon in its leftmost column, so the default rows leave out `state_icon`. Add the token back to a row to restore the inline icon.
