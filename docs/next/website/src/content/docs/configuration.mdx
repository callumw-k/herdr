...
A declared repo owns a workspace. When the focused pane changes directory into a declared repo that no workspace is pinned under, Herdr moves the pane into a new workspace pinned to that repo.
...
A plain entry declares `path` itself. With `children = true` the entry is a projects folder: each immediate child of `path` is a declared repo, so `~/code/projects/budget` and `~/code/projects/todos` each get their own workspace without being listed. The folder itself is not a repo. Deeper declarations win, so an explicit entry for a worktree under one of those children takes precedence over the child.

`prefix+>` adds the active workspace's directory as a plain entry, or removes it when it is already declared. It never removes a `children = true` entry.
...
`ui.pane_borders` accepts `"auto"` (the default: split panes get borders, a single pane keeps a one-row title strip), `"always"` (frame a single pane on all sides), or `"off"`. A full single-pane frame requires `ui.pane_outer_borders = true`, because all its edges are outer edges. Existing boolean values remain valid: `true` means `"auto"` and `false` means `"off"`.
...
The symbols are static. The only motion in the sidebar is the blocked dot, which pulses while an agent is waiting on you. Turn that off with:
...
The leftmost column is reserved for a marker on the focused entry and the Navigate cursor; agent state is carried by `state_icon`, so keep that token when you write custom rows.
...
- `activity` — the first of `terminal_title_stripped`, `pane`, or `agent` that is set.
...
A `text` entry places literal text in a row, for indentation or a custom separator:

```toml
[ui.sidebar.agents]
rows = [
  ["state_icon", "activity"],
  [{ text = "  " }, "state_text", { text = " | ", dim = true }, "agent"],
]
```

Literal text takes the same `fg`, `bold`, and `dim` fields, never accepts `rules`, and gets no ` · ` separator on either side. It is never truncated or dropped when the sidebar is narrow. A row whose only values are literal text disappears, so an indent on an empty row leaves no blank line. Text must be 1 to 32 characters with no control characters.
...
`ui.sidebar.spaces.divider` draws a rule between top-level spaces. The rule sits inside `row_gap`, and raises a zero gap to one row so it has somewhere to live. Worktree children stay packed under their parent either way.
