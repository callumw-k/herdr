...
Herdr はプレフィックスキーを、ワンショットのプレフィックスではなく vim スタイルの固定的なノーマルモードに変えられます。デフォルトでは無効です:
...
`modal = true` にすると、プレフィックスキーを押すとノーマルモードが開き、1 回のコマンド実行後にペインへ戻るのではなく、開いたままになります。修飾なしのキーはそのままコマンドとして機能します。既存の `prefix+X` バインドはすべて、ノーマルモードが開いている間は単独の `X` としても機能します。`keys.enter_insert`(デフォルト `i`)はノーマルモードを抜けて、フォーカスされているペインでの入力に戻ります。`esc` とプレフィックスキーもノーマルモードを抜けます。シーケンスが途中まで入力されている場合、`esc` はまずそのシーケンスをクリアし、抜けずにノーマルモードにとどまります。

モードバーは、モーダル入力が有効な間は `NORMAL` を表示し、シーケンスが途中まで入力されている間は有効な続きのキーを表示します。

### シーケンスバインド

バインドは 2 ステップ以上を連結でき、単一キーのバインドと同じフィールドにスペース区切りで書きます:

```toml
new_workspace = ["prefix+n", "leader w n"]
```

`keys.leader`(デフォルト `"space"`)は、シーケンスの最初のステップとして `leader` トークンを展開します。シーケンスには `keys.modal = true` が必要です。モーダル入力が無効な場合、Herdr は設定読み込み時にシーケンスバインドを診断メッセージとともに拒否します。

ノーマルモードでは単独の文字がそのままコマンドとして機能するため、シーケンスは既存の `prefix+X` バインドがすでに使っている文字から始めることはできません。使い勝手のよい文字の多くはデフォルトで使用済みです: goto の `g`、ワークスペースピッカーの `w`、設定の `s`、デタッチの `q` です。`leader` を起点にシーケンスを組むとこの衝突を避けられるため、先頭の文字を空けない限りは `leader` から始めてください:
...
シーケンスは互いに曖昧な場合も拒否されます。あるシーケンスが別のシーケンスの接頭辞になる場合、また、シーケンスの先頭キーがすでにノーマルモードのキーとして使われている場合です。
