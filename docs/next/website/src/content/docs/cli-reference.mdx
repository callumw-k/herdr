...
herdr workspace set-path <workspace_id> [PATH | --clear]
...
Pin the workspace to a directory with `set-path`. A pinned workspace claims new panes opened under that directory, described in [Concepts](/docs/concepts/). Pass a path to set it, or `--clear` to remove it:

```bash
herdr workspace set-path w1 ~/code/herdr
...
herdr tab floats-toggle [--workspace <workspace_id>] [--toggle|--on|--off]
```
...
`tab floats-toggle` shows or hides the active tab's floating pane layer without closing any float; `--toggle` is the default mode.
...
herdr pane float [--workspace <workspace_id>] [--cwd PATH] [--focus] [--no-focus]
```
...
`pane float` opens a new floating pane on the active tab of the target (or active) workspace; without `--workspace` it fails if no workspace is active. A tab's floats share one region and are laid out inside it under the floating layer's own arrangement, so more than one can be visible at once. `pane split` aimed at a float adds another float. Use `pane focus` to move between floats, the same way you would between tiled panes.

Under a tab arrangement (see [Concepts](/docs/concepts/)), `--split` and `--ratio` on `pane move` and `--ratio` on `pane split` only decide where the pane lands in pane order. The tab is reflowed under its arrangement on the next frame, so the ratio is not kept. Use `layout apply` when you need an exact tree.
