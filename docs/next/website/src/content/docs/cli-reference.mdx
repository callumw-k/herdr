...
herdr pane float-cycle [--workspace <workspace_id>] [--next|--previous]
```
...
`pane float` opens a new floating pane on the active tab of the target (or active) workspace; without `--workspace` it fails if no workspace is active. A tab's floats share one region and are laid out inside it under the floating layer's own arrangement, so more than one can be visible at once; `pane float-cycle` moves focus to the next float in the layer's order, and `--next` is the default direction. Cycling always acts on the active tab, so a `--workspace` other than the active one returns `workspace_not_active`.
