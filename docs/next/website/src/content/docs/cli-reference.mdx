...
`pane float` opens a new floating pane on the active tab of the target (or active) workspace; without `--workspace` it fails if no workspace is active. Floats stack per tab and only the topmost one is visible, so `pane float-cycle` brings the next float to the top; `--next` is the default direction. Cycling always acts on the active tab, so a `--workspace` other than the active one returns `workspace_not_active`.
