...
herdr workspace create [--cwd PATH] [--path PATH] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]
...
`--path` pins the workspace to a directory it claims for pane routing, described in [Concepts](/docs/concepts/). Give `--path` on its own and the first pane starts there too:

```bash
herdr workspace create --path ~/code/herdr --label herdr
```

Change or drop the pin later with `set-path`. Pass a path to set it, or `--clear` to remove it:
...
`pane float` opens a new floating pane on the active tab of the target (or active) workspace; without `--workspace` it fails if no workspace is active. A tab's floats share one region and are laid out inside it under the floating layer's own arrangement, so more than one can be visible at once. Use `pane focus` to move between floats, the same way you would between tiled panes.
