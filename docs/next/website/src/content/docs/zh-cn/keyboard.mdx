...
| 循环窗格排布 | `prefix+a` / `prefix+shift+a` |
...
每个标签页按一种排布摆放窗格: 均分的列、均分的行、网格,或单个堆叠。`prefix+a` 和 `prefix+shift+a` 在它们之间循环,每次都会按窗格原有顺序重建标签页,所以转一圈就会回到你开始时的布局。各种排布的样子见[核心概念](/zh-cn/docs/concepts/)。
