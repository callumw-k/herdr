...
| 循环窗格排布 | `prefix+]` / `prefix+[` |
| 按当前排布新建窗格 | `prefix+enter` |
| 调整大小模式 | `prefix+r` |
| 复制模式 | `prefix+u` |
| 新建浮动窗格 | `prefix+f` |
| 聚焦或隐藏浮动层 | `prefix+ctrl+f` |
| 显示 / 隐藏浮动窗格 | `prefix+shift+f` |
...
每个标签页按一种排布摆放窗格: 均分的列、均分的行、网格,或单个堆叠。`prefix+]` 和 `prefix+[` 在它们之间循环,每次都会按窗格原有顺序重建标签页,所以转一圈就会回到你开始时的布局。各种排布的样子见[核心概念](/zh-cn/docs/concepts/)。

`prefix+v` 和 `prefix+minus` 在添加窗格的同时也会设置排布: `prefix+v` 把标签页切换成均分的列,`prefix+minus` 切换成均分的行,两种情况下整个标签页都会重建。按两次 `prefix+v` 得到的是三个均分的列,而不是嵌套的分割。想在当前形状下再加一个窗格,请用 `prefix+enter`。

重建标签页会丢弃手动调整的大小。你拖动或用 `prefix+r` 调整过的边框会保持到下一次创建或关闭窗格为止,之后由排布的均分大小接管。

浮动层保持自己的排布,与标签页的平铺布局分开。`prefix+]`、`prefix+[`、`prefix+v`、`prefix+minus` 和 `prefix+enter` 都作用于当前持有焦点的那一层: 浮动窗格被聚焦时是浮动层,否则是平铺窗格。

## 复制模式

按 `prefix+u` 让聚焦的窗格进入复制模式。用 `h/j/k/l`、tmux 风格的 `w/b/e` 和 `{`/`}` 移动。按 `/` 或 `?` 向前或向后进行文本搜索,再用 `n` 或 `N` 按相同或相反方向重复搜索。查询包含大写字母时区分大小写,否则不区分。用 `v` 或空格开始选择,用 `y` 或回车复制,用 `q` 或 Esc 不复制直接退出。Esc 会先清除当前选择或搜索,然后才退出。复制模式不会暂停窗格进程:停留在底部时继续跟随输出,进入历史记录后保持当前位置。鼠标拖选可以直接复制,完全不用进入复制模式。
