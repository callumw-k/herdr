...
pub(crate) use self::tokens::{
    ...
    ResolvedToken, ResolvedTokenKind, SpaceTokenContext, PINNED_WORKSPACE_GLYPH,
    PINNED_WORKSPACE_TOKEN,
};
...
pub(crate) fn resolved_token_spans(
    ...
    let fixed_widths = resolved
        ...
        .map(|token| match &token.kind {
            ...
            ResolvedTokenKind::Text(text) => display_width(text),
    ...
    for (position, index) in visible_indices.iter().copied().enumerate() {
        ...
        match &token.kind {
            ...
            ResolvedTokenKind::Text(text) => spans.push(Span::styled(
                text.clone(),
                apply_token_style(Style::default(), token.style),
            )),
        }
