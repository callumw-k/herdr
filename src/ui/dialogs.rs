...
/// The workspace dialog's two input rows. Shared with mouse hit-testing so a
/// click lands on the field the renderer actually drew there.
pub(crate) fn workspace_dialog_field_rects(inner: Rect) -> (Rect, Rect) {
    let rows = rename_modal_rows(inner);
    (
...
fn rename_modal_rows(inner: Rect) -> [Rect; 7] {
    Layout::vertical([
        Constraint::Length(1),
        Constraint::Length(1),
        Constraint::Length(1),
...
pub(super) fn render_rename_overlay(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    if two_field {
        let (name_rect, path_rect) = workspace_dialog_field_rects(inner);
        render_dialog_input(
            frame,
            app,
            name_rect,
...
fn render_dialog_input(
    ...
    // Paths outgrow the modal; elide the middle so the tail being typed and
    // the caret after it stay on screen. The reserved column is the caret's,
    // and it stays blank so the host terminal's inversion is visible there.
    let budget = (area.width as usize)
        .saturating_sub(display_width(&prefix))
        .saturating_sub(1);
    let value = middle_elide(value, budget);
    frame.render_widget(
        Paragraph::new(format!("{prefix}{value}"))
            .style(Style::default().fg(app.palette.text).bg(background)),
        area,
    );

    // The focused field owns the single host cursor, for the same IME reason
    // `render_name_input_field` sets one.
    if focused && area.width > 0 {
        ...
        frame.set_cursor_position((caret_x, area.y));
    }
}
...
mod tests {
    ...
    fn rename_overlay_caret(name: &str) -> Position {
        rename_overlay_caret_in(Mode::RenameTab, name).0
    }
    ...
    fn rename_overlay_anchors_the_host_cursor_to_the_input_caret() {
        ...
        let (caret, buffer) = rename_overlay_caret_in(Mode::RenameTab, "ab");
    ...
    fn rename_overlay_anchors_the_cursor_in_every_rename_mode() {
        ...
        for mode in [Mode::RenameTab, Mode::RenamePane] {
            assert_eq!(
    ...
    #[test]
    fn workspace_dialog_anchors_the_caret_to_the_focused_field() {
        // The two-field workspace dialog carries the same IME requirement as the
        // single-field modes, and only the focused field may claim the one host
        // cursor.
        let caret_for = |field| {
            let mut app = AppState::test_new();
            ...
            app.name_input = "alpha".into();
            app.workspace_dialog_path = "/repo/alpha".into();
            app.workspace_dialog_field = field;

            let mut terminal =
                Terminal::new(TestBackend::new(RENAME_AREA.width, RENAME_AREA.height))
                    .expect("test terminal");
            terminal
                .draw(|frame| render_rename_overlay(&app, frame, RENAME_AREA))
                .expect("workspace dialog should render");
        ...
        let popup = super::centered_popup_rect(
            RENAME_AREA,
            56,
            super::rename_modal_height(Mode::RenameWorkspace),
        )
        .expect("popup fits");
        let inner = Rect::new(popup.x + 1, popup.y + 1, popup.width - 2, popup.height - 2);
        let (name_rect, path_rect) = super::workspace_dialog_field_rects(inner);

        // " name " is six columns, then the five of "alpha".
        assert_eq!(
            caret_for(super::WorkspaceDialogField::Name),
            Position::new(name_rect.x + 11, name_rect.y)
        );
        // " path " is six columns, then the eleven of "/repo/alpha".
        assert_eq!(
            caret_for(super::WorkspaceDialogField::Path),
            Position::new(path_rect.x + 17, path_rect.y)
        );
    }
    ...
    #[test]
    fn a_long_path_keeps_its_tail_and_cursor_inside_the_modal() {
        let mut app = AppState::test_new();
        app.mode = crate::app::Mode::RenameWorkspace;
        app.workspaces = vec![Workspace::test_new("alpha")];
        app.name_input = "alpha".into();
        app.workspace_dialog_path =
            "/home/someone/code/active/herdr-worktrees/workspace-navigator/src".into();
        app.workspace_dialog_field = super::WorkspaceDialogField::Path;
        ...
        let inner = ratatui::widgets::Block::default()
            .borders(ratatui::widgets::Borders::ALL)
            .inner(popup);
        let (_, path_rect) = super::workspace_dialog_field_rects(inner);
        let caret = terminal.get_cursor_position().expect("cursor position");
        let buffer = terminal.backend().buffer();
        let path_row = buffer_row_text(buffer, inner, inner.y + 4);

        assert!(path_row.contains('…'), "expected elision, got {path_row:?}");
        assert!(
            path_row.contains("navigator/src"),
            "the tail being typed must stay visible, got {path_row:?}"
        );
        assert_eq!(
            caret.y,
            inner.y + 4,
            "the caret must sit on the focused path row"
        );
        assert!(
            caret.x < path_rect.right(),
            "the caret must stay inside the field, got {caret:?}"
        );
    }
    ...
    fn rename_overlay_caret_stays_inside_the_input_when_the_name_overflows() {
        ...
        let (caret, buffer) = rename_overlay_caret_in(Mode::RenameTab, &"a".repeat(200));
