...
async fn late_retained_fallback_leaves_all_client_baselines_unchanged() {
    ...
    let inner = linked.panes[0].inner_rect;
    let first_inner_cell =
        usize::from(inner.y) * usize::from(linked.frame.width) + usize::from(inner.x);
    linked.frame.cells[first_inner_cell].hyperlink = Some(0);
