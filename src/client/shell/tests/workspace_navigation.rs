    ...
    for compact in [false, true] {
        for selection_bg in [Color::Reset, Color::Rgb(70, 63, 93)] {
            ...
            for workspace_id in ["ws_1", "ws_2"] {
                ...
                assert_eq!(
                    ribbon_color(&buffer, selected),
                    Some(state.config.palette.accent),
                    "compact={compact}, {workspace_id}"
                );
                // The collapsed strip is too narrow for a fill, so only the wide
                // sidebar carries the selection background.
                if !compact {
                ...
                assert_eq!(ribbon_color(&buffer, untouched), None);
                if workspace_id != "ws_1" {
                    let focused = workspace_rect(&state, &ClientEndpointId::Local, "ws_1");
                    assert_eq!(
                        ribbon_color(&buffer, focused),
                        Some(state.config.palette.text)
                    );
            ...
            assert_eq!(ribbon_color(&buffer, cancelled), None);
            assert_eq!(
        ...
        for terminal_theme in [false, true] {
            ...
            for (endpoint, collision, steps) in [
                ...
                if cols == 44 {
                    ...
                    assert_ne!(buffer[(focused.x + 2, focused.y)].bg, color);
                    assert_eq!(buffer[(focused.x + 2, focused.y)].symbol(), "▌");
                    assert_eq!(buffer[(focused.x + 2, focused.y)].fg, palette.text);
                }
            }
...
fn foreign_preview_blocks_keyboard_actions_but_keeps_active_action_context() {
    ...
    enter_navigation(&mut state);
    let rename = state.handle_input_bytes(b"W");
    assert!(matches!(state.overlay, Some(ClientShellOverlay::Rename(_))));
    assert!(
        matches!(rename.actions.as_slice(), [ClientShellAction::Endpoint { request, .. }]
        if matches!(&request.method, crate::api::schema::Method::WorkspaceGet(_)))
    );
}
...
fn ribbon_color(buffer: &Buffer, rect: Rect) -> Option<ratatui::style::Color> {
    (rect.x..rect.right()).find_map(|x| {
        let cell = &buffer[(x, rect.y)];
        (cell.symbol() == "▌").then_some(cell.fg)
    })
}

/// A pending navigation target wears the accent ribbon until focus lands on it;
/// plain focus wears the text ribbon. Both sidebar widths use the ribbon, so it
/// reads the same whether or not the row also takes a selection fill.
fn assert_local_highlight(state: &mut ClientShellState, selected_id: &str) {
    ...
    for workspace_id in ["ws_1", "ws_2", "ws_3"] {
        ...
        let expected = if workspace_id == selected_id && focused.as_deref() != Some(selected_id) {
            Some(palette.accent)
        } else if Some(workspace_id) == focused.as_deref() {
            Some(palette.text)
        } else {
            None
        };
        assert_eq!(
            ribbon_color(&buffer, rect),
            expected,
